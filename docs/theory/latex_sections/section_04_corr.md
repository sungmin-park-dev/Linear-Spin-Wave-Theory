...
- **Real-time correlation function:** The real-time spin-spin
  ...
  momentum and time: $$\begin{equation}
  ...
  - The spin-spin correlation function under complex conjugation is
    ...
    \end{equation}$$ At this point, the spin-spin correlation transforms
    ...
    \end{equation}$$ where in the second equality we use the identity
    $\langle \hat{A}(0) \hat{B}(t) \rangle = \langle \hat{A}(-t) \hat{B}(0) \rangle$
    for thermal states $\rho = e^{-\beta H}/Z$: $$\begin{equation}
...
- **Static structure factor:** The static structure factor is defined as
  ...
  \end{equation}$$ This equals the equal-time spin-spin correlation
...
- **Dynamical structure factor:** The dynamic structure factor is
  ...
  \end{equation}$$ This quantity is directly related to the scattering
...
- **Real space correlation function in the reference frame:** The
  ...
  \end{equation}$$ where $\widetilde{S}^{\alpha}_{i}$ represents the
...
- **Spectral function:** The spectral function is defined as:
  ...
  \end{equation}$$ where $G^{\alpha\beta}_{\rm R}(\mathbf{k}, \omega)$
  is the retarded Green's function: $$\begin{equation}
  ...
  \end{equation}$$ The spectral function provides information about the
...
$$\begin{equation}
    \mathcal{C}^{\alpha\beta}(\mathbf{k}, t) = \frac{1}{L} \sum_{i, j} e^{-i \mathbf{k} \cdot (\mathbf{r}_{i} - \mathbf{r}_{j})} \langle \hat{S}_{i}^{\alpha}(t) \hat{S}_{j}^{\beta}(0) \rangle,
\end{equation}$$ where $\alpha, \beta = x, y, z$ or $\pm, 0$ and $L$ is
the number of lattice sites for spins. To proceed, we express the
$\alpha$-component of the spin operator as $$\begin{equation}
    \hat{S}^{\alpha}_{i} 
...
\end{equation}$$ If the indices $\alpha$ are $x,y,z$,
$\mathbf{U}_{i} = \mathbf{R}_{i} \mathbf{C}$. Substituting into the
spin-spin correlation function, we obtain $$\begin{align}
    ...
    &\quad + \frac{1}{\sqrt{2}} \Bigg[ 
        ...
        \Bigg\rangle 
    \Bigg].
\end{align}$$ In LSWT, which is a quadratic bosonic system, odd-order
bosonic correlation functions vanish, so the terms in the second line
are zero. Thus, $$\begin{align}
    \langle S_i^{\alpha}(t) S_j^{\beta}(0) \rangle
    &= \frac{1}{2} \Bigg\langle 
        \big( \bar{u}^{\alpha}_{i} \widetilde{S}_{i}^{+}(t) + u^{\alpha}_{i} \widetilde{S}_{i}^{-}(t) \big) 
        \big( \bar{u}^{\beta}_{j} \widetilde{S}_{j}^{+}(0) + u^{\beta}_{j} \widetilde{S}_{j}^{-}(0) \big) 
...
transversal fluctuations). $$\begin{equation}
    \mathsf{U}^{\alpha}_{i} 
    \equiv 
    \begin{pmatrix}
         \bar{u}^{\alpha}_{i} & 0 \\ 0 & u^{\alpha}_{i} 
    ...
    \begin{pmatrix}
        ...
        \langle \widetilde{S}^{-}_{i}(t) \widetilde{S}^{+}_{j}(0) \rangle
    \end{pmatrix}
\end{equation}$$ The Holstein-Primakoff (HP) operators are
$$\begin{equation}
...
\end{equation}$$ with $\hat{n}_i = \hat{b}_i^{\dagger} \hat{b}_i$ being
...
for a time-independent Hamiltonian $\hat{H}$, $$\begin{equation}
    ...
    \end{cases}
\end{equation}$$ where $\hat{U}(t) = e^{-i \hat{H} t}$ is the
...
magnetic sublattices: $$\begin{align}
    \mathcal{C}^{\alpha\beta}(\mathbf{k}, t) 
...
over sublattice correlation functions: $$\begin{align}
    \mathcal{C}^{\alpha\beta}(\mathbf{k}, t) 
    = \frac{1}{m_s} \sum_{\mu, \nu = a, b, \dots} 
    \mathcal{C}_{\mu\nu}^{\alpha\beta}(\mathbf{k}, t),
\end{align}$$ where $m_s$ is the number of magnetic sublattices in the
...
$\mathsf{C}^{\alpha\beta}(\mathbf{k}, t)$: $$\begin{equation}
    \begin{split}
        \mathcal{C}_{\mu\nu}^{\alpha\beta}(\mathbf{k}, t) 
        &= \sum_{(m,n)_{\mu,\nu}} 
    ...
    \end{split}
\end{equation}$$ where the index set
$(m,n)_{\mu,\nu} = \{ (\mu, \nu), (m_s + \mu, \nu), (\mu, m_s + \nu), (m_s + \mu, m_s + \nu) \}$
...
\end{equation}$$ where $\mathsf{T}_{\mathbf{k}}$ is a para-unitary
...
which can be expressed explicitly as: $$\begin{equation}
    \mathsf{R}^{\alpha}_{\mathbf{k}} 
    ...
    \end{pmatrix},
\end{equation}$$ where the diagonal matrices $\mathsf{s}$ and
$\mathsf{u}^{\alpha}$ are defined as: $$\begin{equation}
    \begin{cases}
    ...
    \end{cases}
\end{equation}$$ where $\mathsf{u}^{\alpha}$ contains the
...
magnon correlations and is given by: $$\begin{equation}
    \mathsf{N}_{\mathbf{k}}(t) 
    = 
    \begin{pmatrix}
    ...
    \end{pmatrix},
\end{equation}$$ where $\mathsf{I}$ is the $m_s \times m_s$ identity
...
expressed as $$\begin{align}
    ...
    \bigg], 
    \nonumber
\end{align}$$ where the spin operators are transformed into bosonic
...
bosonic correlation matrix, yields $$\begin{align}
        \frac{1}{N} \sum_{i,j}   
        ...
        = &
        \big\langle \Psi_{i}^{\phantom \dagger}(t) \Psi_{j}^{\dagger}(0) \big\rangle.
    \end{align}$$ Similarly, the spin-spin correlation function in real
space is obtained as $$\begin{equation}
...
\end{equation}$$ derived by applying the Fourier transform to the
...
\end{align}$$ where $\mathsf{T}_{\kvec}$ and $\mathsf{N}_{\kvec}(t)$ are
defined above. The second term, representing the density operator in
momentum space, is $$\begin{equation}
...
\end{equation}$$ where $\hat{n}_{\kvec, \mu}$, the density fluctuation
operator at momentum $\kvec$, is $$\begin{align*}
    \hat{n}_{\kvec} 
    = & \frac{1}{N} 
...
\end{align*}$$ which vanishes for a free system unless
$\kvec = \kappa \in \mathbf{G}$ due to momentum conservation.
...
$$\begin{equation}
    ...
    \right],
\end{equation}$$ where the first term describes time-dependent magnon
...
\label{eq: matrix form of spin-spin correlation matrix}
    ...
    \mathsf{U}^{\beta},
\end{equation}$$ with elements computed as $$\begin{align*}
    \mathsf{C}^{\alpha\beta} (\kvec, t) = & 
    \begin{pmatrix}
    ...
    \end{pmatrix}.
\end{align*}$$ The elements of $\mathsf{C}^{\alpha\beta}(\kvec, t)$ for
indices $(m,n)_{\mu,\nu}$ are $$\begin{align*}
    \begin{array}{crl}
        ...
        \big\langle b_{-\mathbf{k}\mu}^{\dagger} (t) b_{-\mathbf{k}\nu} (0) \big\rangle.
    \end{array}
\end{align*}$$ Summing $\mathsf{C}^{\alpha\beta}(\kvec, t)$ over
...
\end{equation}$$ The spin structure factor is obtained by summing over
all sublattices:
$\mathcal{S}^{\alpha\beta} (\mathbf{k}, t) = \mathcal{C}^{\alpha\beta} (\kvec, t) = \frac{1}{m_s} \sum_{\mu,\nu} \mathcal{C}_{\mu\nu}^{\alpha\beta} (\kvec, t)$.
...
\end{equation}$$ This quantity $\mathcal{S}^{\alpha\beta} (\mathbf{k})$
is equal to $\mathcal{C}^{\alpha\beta}(\kvec, t)$, which we've found the
matrix form in a previous equation: $$\begin{equation}
    \mathcal{S}^{\alpha\beta} (\mathbf{k}) = 
    \sum_{m,n} 
    \big[ \mathsf{U}^{\alpha} 
    \mathsf{S}_{\kvec}^{\phantom \dagger} 
...
\end{equation}$$ where $\mathsf{U}^{\alpha}$ and $\mathsf{U}^{\beta}$
are spin projection operators, $\mathsf{S}_{\kvec}$ and
$\mathsf{T}_{\kvec}$ are transformation matrices, and
$\mathsf{N}_{\kvec}$ accounts for the diagonalized magnon correlation
...
\end{align*}$$ where the matrix elements are evaluated in the rotated
spin basis.
...
\end{equation}$$ This formulation respects the directional constraints
intrinsic to neutron scattering physics.

### Dynamic structure factor

The dynamic structure factor is defined as $$\begin{equation}
    \mathcal{S}^{\alpha\beta}(\kvec, \omega) 
    = \frac{1}{2\pi} \int_{-\infty}^{\infty} d t \, e^{i \omega t} \mathcal{C}^{\alpha\beta} (\kvec, t).
\end{equation}$$ From the LSWT, we consider $$\begin{align*}
...
\end{align*}$$ However, the integral yields $\delta$-function. So we
introduce the decay factor $\eta > 0$ so that
$\omega \rightarrow \omega + i \eta$: $$\begin{equation}
    g_{\kvec} ( \omega ; \eta ) 
    \equiv \int_{0}^{\infty} dt \, e^{ - [ \eta  + i( E_{\kvec} - \omega ) ] t} 
    = \frac{1}{\eta + i ( E_{\kvec} - \omega )}
\end{equation}$$ Using this notation, we have $$\begin{align}
...
\end{align}$$ where $\mathcal{F}_{\kvec}(\omega; \eta)$ is Lorentzian
and its FWHM is $2\eta$. $$\begin{equation}
    \begin{split}
        ...
        \end{pmatrix} 
    \end{split}
\end{equation}$$ Thus, in LSWT, the dynamical spin structure factor can
be computed from the matrix multiplication: $$\begin{equation}
    \mathcal{S}^{\alpha\beta}_{\mu\nu} (\kvec, \omega)
    =
    \sum_{(m,n)_{\mu,\nu}} 
    \left[  \mathsf{R}^{\alpha}_{\kvec} \mathsf{T}_{\kvec}
    ...
    \mathsf{T}_{\kvec}^{\dagger} [\mathsf{R}^{\beta}_{\kvec}]^{\dagger} \right]_{m,n}.
\end{equation}$$ Here, the fourier transformation of diagonal matrix
$\mathsf{N}_{\kvec} (t)$ is given by $$\begin{equation}
    \mathcal{F} [\mathsf{N}_{\kvec}](\omega; \eta)
    \equiv 
    \begin{pmatrix}
        ( \mathsf{I} + \mathsf{n}_{\kvec} )    \mathcal{F}_{ \kvec} (  \omega; \eta  )  & 0 \\
...
Spin-Wave Theory (LSWT): $$\begin{equation}
    \mathcal{A}^{\alpha\beta}(\kvec, \omega) = - \frac{1}{\pi} \mathrm{Im} [G^{\alpha\beta}_{\rm R} (\kvec, \omega)],
\end{equation}$$ where $G^{\alpha\beta}_{\rm R} (\kvec, \omega)$ is the
retarded Green's function, defined as $$\begin{equation}
...
\end{equation}$$ The ratared Green's function can be rewritten in terms
of the correlation function $$\begin{equation}
    G^{\alpha\beta}_{\rm R} (\kvec, \omega) 
...
\end{equation}$$ For spin components $\alpha, \beta = x, y, z$, the
retarded Green's function is expressed as the Fourier transform of the
imaginary part of the correlation function: $$\begin{equation}
    G^{\alpha\beta}_{\rm R} (\kvec, \omega) 
    = 2 \int_{0}^{\infty} dt \, \mathrm{Im} [\mathcal{C}^{\alpha\beta}(\kvec, t) ] e^{\ri \omega t}.
\end{equation}$$ The spectral function is then obtained as
$$\begin{equation}
    \begin{split}
        ...
        \right]_{m,n}
    \end{split}
\end{equation}$$ where the function $\mathcal{G}_{\kvec}(\omega ; \eta)$
...
\end{align*}$$ Thus, the spectral function in LSWT can be computed from
the product of the matrices. $$\begin{equation}
    \mathcal{A}^{\alpha\beta}(\kvec, \omega) 
    = - \frac{2}{\pi} \mathrm{Im} 
    \sum_{m,n}
    \left[ 
...
$\mathcal{G} [ \mathsf{N}_{\kvec} ] (\omega ; \eta)$ is defined as
$$\begin{equation}
    \mathcal{G} [ \mathsf{N}_{\kvec} ] (\omega ; \eta)
    = 
    \begin{pmatrix}
