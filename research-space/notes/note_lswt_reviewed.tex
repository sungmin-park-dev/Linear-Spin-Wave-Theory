...
\REVIEW{A3: $\hat{\bf e}_{j}$ here should be $\hat{\bf e}_{j}^{0}$ to match Sec~1.1 notation. The superscript 0 is dropped inconsistently.}
...
Note that the constant term can be expressed as $\frac{1}{2} \sum_{\mathbf{k} \in \mathrm{FBZ}}  C_{\mathbf{k}}  = - \frac{1}{4} \Tr[\mathsf{H}_{\mathbf{k}}]$
...
\REVIEW{A14: $\mathcal{F}_k$ is defined for scalar $e^{-iEt}$ but $N_k(t)$ has different time dependence in upper-left and lower-right blocks. Make the connection explicit.}
\begin{equation}
    \begin{split}
...
\REVIEW{A15: Formula $G_R = 2\int \mathrm{Im}[C] e^{i\omega t} dt$ holds only for $\alpha,\beta=x,y,z$. The $\pm,0$ case needs $G_R = (1/i)\int [C^{\alpha\beta} - C^{\bar{\beta}\bar{\alpha}}(-k,-t)] e^{i\omega t} dt$. State explicitly if needed.}
\begin{equation}
    G^{\alpha\beta}_{\rm R} (\kvec, \omega) 
...
\REVIEW{B9: Broken/orphaned text from incomplete edit (`ho in the lattice [cite] However,'). Remove or complete this sentence fragment.}
