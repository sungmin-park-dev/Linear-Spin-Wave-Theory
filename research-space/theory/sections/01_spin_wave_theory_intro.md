...
$$
\begin{equation}
\label{eq: General model Hamiltonian for spin wave theory}
    \hat{H} 
...
Eqn. (Eq. \ref{eq: General model Hamiltonian for spin wave theory}).
...
\begin{equation}
    ...
    \end{equation}
$$ where the exchange energy is composed of inter-site
    interactions (first term) and single-ion anisotropies (second term).
    The exchange matrix ${\bf J}_{ij}$ can represent various types of
    interactions including isotropic exchange, Dzyaloshinskii-Moriya
    interactions, and anisotropic exchanges such as Kitaev interactions.
    The single-ion anisotropy term, for example
    $- A \sum_{i} (S^{x}_{i})^{2}$, describes easy-axis anisotropy along
    $x$.
...
\[Eqn. (Eq. \ref{eq: General model Hamiltonian for spin wave theory})\] can be
...
$$\label{eq: LSWT boson expansion even/odd}
    ...
    \end{align}
$$ and substituting the following spin-to-boson
transformation: $$
\begin{equation}
...
Eqn. (Eq. \ref{subeq: even bosonic terms in SWT}) are: $$
\begin{equation}
    \begin{split}
...
Hamiltonian contributions, $H_{2}$ and $H_{4}$, as: $$
\begin{align}
...
\end{align}
$$ Note that if the anisotropic couplings are absent in the
...
Eq. (Eq. \ref{eq: expressions for exchange couplings}). Furthermore, when
...
Eqn. (Eq. \ref{eq: SWT bosonic H2 in real space}) is $$
\begin{equation}
    \begin{split}
        H_{2} = & \sum_{ij}
...
\end{align}
$$ In the first line, the first term always vanishes if
...
\begin{align*}
    ...
    = 0.
\end{align}
$$ These identities can be represented in a compact matrix
form: $$
\begin{equation}
    \begin{pmatrix}
...
\end{align}
$$ Note that the two diagonal matrices can be mapped to each
...
\end{align}
$$ where $E_{\mathbf{k}} \ge 0$ for all
...
$\mathsf{T}_{\mathbf{k}}$ (we will suppress the $\mathbf{k}$ subscript in this
section for clarity, so $\mathsf{H}_{\mathbf{k}} \rightarrow \mathsf{H}$,
etc.) such that $$
\begin{equation}
...
\end{align}
$$ where we identify the diagonal energy matrix as
